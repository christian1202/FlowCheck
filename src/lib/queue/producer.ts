export async function enqueueSheetSync(eventId: string) {
  ...
  // @ts-expect-error - Cloudflare Queues binding injected by OpenNext/Wrangler
  const queue = process.env.SHEETS_SYNC_QUEUE || (globalThis as unknown as { SHEETS_SYNC_QUEUE?: { send: (msg: unknown) => Promise<void> } }).SHEETS_SYNC_QUEUE;
