...
The architecture is entirely serverless and edge-native, leveraging Cloudflare Pages (via OpenNext) for hosting the Next.js application, Supabase for PostgreSQL and authentication, and Brevo for transactional email. This eliminates the need for persistent worker containers or paid hosting tiers.

Heavy background tasks—specifically, batch-syncing attendance data to Google Sheets without exceeding API rate limits—are handled natively by Cloudflare Queues and Cloudflare Cron Triggers, providing enterprise-grade reliability at zero cost.
...
graph TB
    ...
    subgraph "Edge Layer (Cloudflare)"
        C["Cloudflare Edge<br/>DDoS Protection + WAF + Cache API Rate Limiting"]
    end
    ...
    subgraph "Queue Layer"
        J["Cloudflare Queues<br/>(sheets-sync)"]
    end

    subgraph "Worker Layer (Cloudflare Workers)"
...
| Email | Brevo | 300 emails/day | 3x more emails than Resend's free tier. |
...
| Background | Cloudflare Queues | 1M ops/month | Native batching, replaces BullMQ/Redis. |
...
| Cloudflare Workers | 100,000 requests/day | Extremely generous. Supports ~100K API calls/day. |
| Cloudflare Workers CPU | 10ms CPU time/invocation | **Risk Area:** QR generation (~5ms) fits, but must be monitored. If it exceeds 10ms, QR generation moves to a Queue consumer. Network I/O does not count towards CPU time. |
| Cloudflare Queues | 1,000,000 operations/month | ~33,000 queue messages/day. Plenty for syncing. |
| Supabase | 500MB DB, 50K MAU | Will comfortably hold >100,000 attendee records. |
| Brevo | 300 emails/day | **Bottleneck:** Caps max registrations at ~290/day. |
...
| **Registration** | Attendee Browser → Edge WAF → App Middleware → Server Action → DAL → Daily Cap Check → QR Gen → Brevo API (Sync or Queue) → Supabase (INSERT) |
| **Email Queue Sync** | Cloudflare Cron Trigger → Supabase (SELECT email_sent = false) → Brevo API (Sync) → Supabase (UPDATE) |
...
- **ADR-2: Brevo over Resend.** Brevo provides 300 emails/day on their free tier, compared to Resend's 100/day. This triples the daily registration capacity for a free-tier app.
- **ADR-3: Cloudflare Queues over BullMQ/Upstash.** Cloudflare Queues provides native batching (perfect for the Google Sheets API limit) and 1M free operations/month, eliminating the need for a separate Redis instance and a paid worker container.
...
- **ADR-6: Pure JS QR Generation (No `sharp`).** Cloudflare Workers do not support native C++ modules like `sharp`. We use the `qrcode` library to generate a pure PNG buffer or SVG.
...
2. **Growth ($14/mo):**
   - Cloudflare Workers Paid ($5/mo): 10M requests, 30ms CPU time.
   - Brevo Starter ($9/mo): 5,000 emails/month (removes daily limit).
3. **Scale ($39/mo):**
...
# Email Safety Cap
DAILY_EMAIL_LIMIT=290                  # Stays under Brevo's 300/day limit
```
